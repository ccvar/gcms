...
本文记录把当前单站 CMS 升级为多站点 / 站群管理的第一版设计。当前阶段只作为方案草稿，用于审核和确认细节，尚不代表已经进入实现。
...
1. 打开或创建 `system.db`。
2. 执行 `system.db` 迁移。
3. 如果检测到旧单站结构，创建默认站点并迁移原 `cms.db`、`uploads/` 和管理员凭据。
4. 读取所有站点记录。
...
```text
版本 A：system.db + 默认站点兼容 + 平台 session
版本 B：站点运行时池 + Host 匹配 + 站点列表 / 域名管理
版本 C：平台入口自动化 API + 管理型 AI 接入包
版本 D：后台预览入口
版本 E：后台路径完全前缀化
版本 F：可选目录整理、跨站增强、角色权限
```
...
7. 当前单站数据迁移时，是移动到 `data/sites/main/`，还是保留原路径作为默认站点？
   - 推荐最终结构移动到 `data/sites/main/`，但迁移时需要自动备份。
