...
func platformKitReadme(opts automationSkillOptions) string {
	...
	lines = append(lines,
		...
		"- 不要删除站内内容，不要直接改管理员账号、密码、系统更新、Cloudflare 配置或密钥本身。初始密码只能交给 Pilot 原生界面。",
...
func platformAssistantBriefMarkdown(opts automationSkillOptions) string {
	...
	lines := []string{
		...
		"- 不要删站内内容，不要直接碰账号密码、更新、部署或密钥。初始密码只能由 Pilot 原生界面设置。",
...
func platformSkillMarkdown(apiBase string) string {
	...
	return strings.Join([]string{
		...
		"- `ui_only: true` 的能力不得从脚本执行。特别是初始密码设置只有 Pilot 原生界面可调用。",
