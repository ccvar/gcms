...
func platformSkillAgentYAML() string {
	return strings.Join([]string{
		...
		"default_prompt: First run 'node scripts/gcms.js capabilities'. For management writes always run the matching *-plan command, show its impact, and wait for explicit approval before execution. Never ask for an admin password; Pilot UI handles required unlocks. Default content to drafts; never publish without explicit approval.",
	}, "\n") + "\n"
}
...
func platformKitReadme(opts automationSkillOptions) string {
	...
	lines = append(lines,
		...
		"- 站点创建、修改、归档删除、主题和域名配置必须先运行对应 `*-plan` 命令；把 normalized_input、impact、warnings 原样展示给用户，收到明确确认后才运行实际命令。",
		...
		"- GCMS 域名接口只写内部主域名/跳转域名；DNS、Cloudflare、Caddy 和 HTTPS 是 Pilot 外部编排，不得声称 GCMS 已自动完成。",
		"- 不要删除站内内容，不要直接改管理员账号、密码、系统更新、Cloudflare 配置或密钥本身。初始密码没有 HTTP 写入口，只能由 Pilot 通过服务器上的 GCMS 专用 CLI 设置。",
...
func platformAssistantBriefMarkdown(opts automationSkillOptions) string {
	...
	lines := []string{
		...
		"- 管理写操作必须先运行对应 `*-plan`；向用户展示影响与警告，用户明确确认后才执行，并为同一次重试复用稳定 request-id。",
		"- 主题由 AI 根据主题接口返回的真实 description/category/layout 推荐，不得编造主题 ID。",
		"- 域名接口只更新 GCMS 内部配置；DNS、Cloudflare、Caddy、HTTPS 交给 Pilot，不能把 external_requirements 当成已完成。",
		"- 不要删站内内容，不要直接碰账号密码、更新、部署或密钥。初始密码没有控制 API 写入口，只能由 Pilot 通过服务器上的 GCMS 专用 CLI 设置。",
...
func platformSkillMarkdown(apiBase string) string {
	...
	return strings.Join([]string{
		...
		"4. 站点/主题/域名写操作先运行对应 `*-plan`，展示服务端返回的 normalized_input、impact、warnings。",
