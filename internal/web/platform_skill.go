...
func platformSkillAgentYAML() string {
	return strings.Join([]string{
		...
		"default_prompt: First run 'node scripts/gcms.js capabilities'. For a landing, campaign, or content-hub page, stay in the Pilot conversation and run page-context before page-capabilities; inherit the site theme, bind real site data, preserve ETag/base revision/idempotency, and return a private preview after each meaningful revision. Always show the matching plan before a sensitive write. Never ask for an admin password or approval token; Pilot native UI handles target-bound unlocks. Default content to drafts; never publish without explicit approval.",
	}, "\n") + "\n"
}
...
func platformSkillMarkdown(apiBase string) string {
	...
	return strings.Join([]string{
		...
		"7. 宣传页、专题页或内容聚合页始终在 Pilot 对话中完成：先运行 `page-context --site <站点> --lang <语种>`，再运行 `page-capabilities --site`；已有页面用 `page-projects --site <站点> --lang <语种> --slug <slug>` 发现真实 project id/ETag。默认继承当前主题、绑定真实数据，按 page-get → revision/app upload → validate → build → preview → publish-plan 操作；把 ready build.id 传给 preview 与后续发布链路，确保它们绑定同一产物和数据快照。每次实质修改都返回预览和摘要。",
		"",
		"## 站点选择规则",
		"",
		...
		"- 非默认站未绑定任何已启用域名时没有真实线上入口：页面发布/回滚仍需用户显式确认、If-Match 与 request-id，但无需后台密码；以 page-capabilities 与 publish-plan 的实时 `requires_unlock` / `requires_approval_token` 为准。",
		"- 正式命令返回 `unlock_required` 时，助手回复必须完整保留 `unlock_required`、operation、unlock_challenge、site/page/project/revision、ETag、request-id 与 admin_path，以触发 Pilot 原生密码框；验证后只重试完全相同请求。",
		"- AI/技能脚本不得接收后台密码或 approval token。页面原生令牌由服务端绑定 operation+site+page+project+revision+ETag+request-id，并在同一发布请求内转换和消费。",
		...
		"5. 需要封面或正文图片时，先把图片转成 WebP（.webp）；多个文件必须通过一条 `node scripts/gcms.js upload --site <站点> <文件1> <文件2> ...` 批量上传，由脚本限并发并重试瞬时网络失败，禁止让模型逐张循环重试；拿返回 `url` 写入字段。",
		...
		"8a. 如果紧邻的上一轮已经对一组明确 ID 完成审计或预览，而用户本轮只是明确要求发布同一组内容，不要重复全量审计，也不要先手工逐条读取 ETag。直接逐条运行 `update <collection> <id> '{\"status\":\"published\"}' --site <slug>`；脚本会在每次 PATCH 前读取该条最新状态与 ETag。只有目标状态或 ETag 已变化时，才重新复核受影响的单条内容。",
		"8aa. 批量发布要分成可核对的小批次；瞬时网络读取失败最多重试 2 次并短暂退避，之后停止该条并明确报告未处理 ID、已完成 ID 和下一步，不得无限等待或重复写入已成功项目。",
		...
		"8e. 页面工程的每个写操作必须使用 page-get 返回的 ETag、明确 base_revision_id 和稳定 request-id；409 后重新读取并合并，禁止覆盖人工修改。",
		...
		"- `node scripts/gcms.js page-context --site blog --lang zh`（页面创作第一步）",
		"- `node scripts/gcms.js page-capabilities --site blog`",
		"- `node scripts/gcms.js page-projects --site blog --lang zh --slug campaign-2026`（发现已有页面工程）",
		"- `node scripts/gcms.js page-get --site blog 42`（返回 `_protocol.etag` 与后台打开路径）",
		"- `node scripts/gcms.js page-components --site blog` / `page-data-sources --site blog --lang zh` / `page-binding-preview --site blog @binding.json`",
		"- `node scripts/gcms.js page-revisions --site blog 42 --limit 100` / `page-revision --site blog 42 5` / `page-assets --site blog 42`",
		"- `node scripts/gcms.js page-restore --site blog 42 --revision-id 3 --etag '<copy _protocol.etag verbatim>' --request-id page-restore-003 --confirm true`",
		"- `node scripts/gcms.js page-app-source-read --site blog 42 src/app.js --revision-id 5`",
		"- `node scripts/gcms.js page-app-source-edit --site blog 42 src/app.js @app.js --base-revision-id 5 --etag '<copy _protocol.etag verbatim>' --request-id page-source-006 --confirm true`",
		"- `node scripts/gcms.js page-capability-list --site blog 42` / `page-capability-request ...` / `page-capability-grant ...` / `page-capability-revoke ...`（批准挑战只能交给 Pilot 原生界面）",
		"- `node scripts/gcms.js page-build --site blog 42 --revision-id 5 --etag '<copy _protocol.etag verbatim>' --request-id page-build-005`",
		"- `node scripts/gcms.js page-build-get --site blog 42 7` / `page-publications --site blog 42 --limit 100`",
		"- `node scripts/gcms.js page-publish-plan --site blog 42 --revision-id 5 --build-id 7 --etag '<copy _protocol.etag verbatim>'`（预检只带 If-Match，不传 request-id）",
		"- `node scripts/gcms.js page-publish --site blog 42 --revision-id 5 --build-id 7 --etag '<copy _protocol.etag verbatim>' --request-id page-publish-005 --confirm true`（始终显式确认；只有默认站或已绑定域名的站点需要 Pilot 原生密码验证）",
		...
		"- `node scripts/gcms.js search-stats --site blog --days 28 --limit 100 --group query`（stats:read：按搜索词聚合，返回点击、曝光、CTR 与平均排名）",
		"- `node scripts/gcms.js search-stats --site blog --days 28 --group page --compare`（按页面聚合并附带紧前等长区间对比：追加 prev_clicks/prev_impressions/prev_ctr/prev_position）",
		"- `node scripts/gcms.js search-stats --site blog --days 28 --group date`（每日搜索趋势；date 不与 compare 同用）",
