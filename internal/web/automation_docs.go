	...
	paths := map[string]any{
		...
		"/stats/search": map[string]any{
			"get": map[string]any{
				...
				"description": "查询参数：days=28（钳制 1..90）、limit=100（钳制 1..1000）、compare=1（可选）、group=query_page|query|page|date|total（默认 query_page）、fresh=1（可选，绕过 1 小时缓存）。每行返回 clicks、impressions、ctr、position，并按 group 附带 query/page/date；total 返回整个区间的精确汇总。compare=1 时服务端另拉紧前等长区间并按当前 group 合并，每行追加 prev_clicks/prev_impressions/prev_ctr/prev_position；date 不支持 compare。未接入 Search Console 时返回 400 search_console_not_connected。典型用法：找高曝光低点击、排名 4~20 的机会词，识别增长/下滑页面，并用 date 观察趋势。",
				"responses":   map[string]any{"200": map[string]any{"description": "OK"}},
			},
		},
		"/stats/traffic": map[string]any{
			"get": map[string]any{
				...
				"description": "查询参数：days=7（钳制 1..90，统计截至昨天的完整自然日）、fresh=1（可选，绕过 1 小时缓存）。优先按站点绑定的 GA4 数据流过滤；旧配置无数据流时按该站点正式域名过滤。返回 {ok,days,property,scope_host,scope_type,scope_stream_id,active_users,sessions,engagement_rate,average_session_duration}。scope_type=stream 时 scope_host 仅标识当前网站，不是额外域名过滤。未接入 Google Analytics 时返回 400 analytics_not_connected。",
				"responses":   map[string]any{"200": map[string]any{"description": "OK"}},
			},
		},
		"/stats/pages": map[string]any{
			"get": map[string]any{
				...
				"description": "查询参数：days=7（钳制 1..90，统计截至昨天的完整自然日）、limit=50（钳制 1..1000）、fresh=1（可选，绕过 1 小时缓存）。GA4 优先按站点绑定的数据流过滤；旧配置无数据流时按该站点正式域名过滤。按 pagePath 维度返回 {ok,days,property,scope_host,scope_type,scope_stream_id,rows:[{path,active_users,sessions,engagement_rate,average_session_duration}]}（活跃用户降序）。未接入 Google Analytics 时返回 400 analytics_not_connected。典型用法：识别主要流量页面，并配合 /stats/search 定位要优化的旧文。",
				"responses":   map[string]any{"200": map[string]any{"description": "OK"}},
			},
		},
		"/stats/analytics": map[string]any{
			"get": map[string]any{
				...
				"description": "查询参数：group=sources|geography|devices|trend（默认 sources）、days=7（钳制 1..90，统计截至昨天的完整自然日）、limit=50（钳制 1..1000）、fresh=1（可选）。优先按站点绑定的 GA4 数据流过滤；旧配置无数据流时按该站点正式域名过滤。返回 scope_host、scope_type、scope_stream_id、dimensions 及 rows；每行包含与 dimensions 同序的 values，以及 active_users、sessions、engagement_rate、average_session_duration。sources=渠道+来源/媒介，geography=国家+地区，devices=设备类型+系统+浏览器，trend=日期升序。结果按端点参数与统计范围缓存 1 小时。",
				"responses":   map[string]any{"200": map[string]any{"description": "OK"}},
			},
		},
...
func automationSimilarOperation(col automationCollection) map[string]any {
	return map[string]any{
		"summary":     "查重：按标题找站内近似" + col.label + "（" + col.path + ":read）",
		"description": "写新内容前先查重，避免重复选题。查询参数：title（必填）、lang（可选，缺省全语种）、limit（1..20，默认 5）。基于 FTS5 做标题近似匹配（含已发布 + 草稿），返回 {ok,rows:[{id,title,slug,status,lang,score}]}，score 归一化 0..1（1 = 最相似）。score 较高（如 ≥0.6）时应改为更新那篇旧文，而不是再写一篇。",
		"operationId": "similar" + automationOperationSuffix(col.path),
		"tags":        []string{col.label},
		"parameters": []map[string]any{
			{"name": "title", "in": "query", "required": true, "schema": map[string]any{"type": "string"}, "description": "拟发内容的标题。"},
			{"name": "lang", "in": "query", "schema": map[string]any{"type": "string"}, "description": "限定语种；缺省或 all 为全语种。"},
			{"name": "limit", "in": "query", "schema": map[string]any{"type": "integer", "default": 5, "minimum": 1, "maximum": 20}},
		},
		"responses": map[string]any{"200": map[string]any{"description": "OK"}},
	}
}
	...
	return map[string]any{
		...
		"ContentInput": map[string]any{
			...
			"properties": map[string]any{
				...
				"robots_override": map[string]any{
					"type": "string", "example": "noindex, follow",
					"description": "单篇 SEO：覆盖该页 robots meta（留空/空串 = 用默认）。",
				},
				"canonical_override": map[string]any{
					"type": "string", "example": "https://example.com/original-article",
					"description": "单篇 SEO：覆盖该页 canonical URL。必须是 http(s) 绝对 URL，否则 422 invalid_canonical；空串 = 清除覆盖。",
				},
			},
		},
...
func automationSkillMarkdown(apiBase string) string {
	return strings.Join([]string{
		...
		"- `node scripts/gcms.js similar posts --title \"拟发标题\" --lang zh`（发文前查重，collection 可省略默认 posts）",
		...
		"- `node scripts/gcms.js search-stats --days 28 --group page --compare`（附带紧前等长区间对比）",
		"- `node scripts/gcms.js search-stats --days 28 --group date`（每日趋势）",
		"- `node scripts/gcms.js traffic-stats --days 7`",
		"- `node scripts/gcms.js page-stats --days 7 --limit 50`",
		"- `node scripts/gcms.js analytics-stats --days 30 --limit 200 --group sources`（GA 渠道与来源 / 媒介）",
		"- `node scripts/gcms.js analytics-stats --days 30 --limit 200 --group geography`（GA 国家 / 地区与区域）",
		"- `node scripts/gcms.js analytics-stats --days 30 --limit 200 --group devices`（GA 设备、系统与浏览器）",
		"- `node scripts/gcms.js analytics-stats --days 30 --limit 100 --group trend`（GA 每日趋势）",
		"- `node scripts/gcms.js tg-stats`（Telegram 频道订阅数）",
		"",
		"## 发文前查重（similar）",
		"",
		"写新文章前先 `node scripts/gcms.js similar posts --title \"拟发标题\"`（`GET /{collection}/similar?title=...`），",
		"基于站内 FTS5 索引做标题近似匹配（含已发布 + 草稿），返回 `{ok,rows:[{id,title,slug,status,lang,score}]}`，",
		"score 归一化 0..1（1 = 最相似）。示例返回：",
		"",
		"```json",
		`{"ok":true,"rows":[{"id":42,"title":"GCMS 入门教程","slug":"gcms-guide","status":"published","lang":"zh","score":0.87}]}`,
		"```",
		"",
		...
		"## 发布质量门（posts 专属）",
		"",
		"通过自动化 API 把 posts 的 `status` 置为 `published`（创建即发布 / 更新改状态）会过一道硬校验：",
		"正文有效长度 ≥400 字（去 Markdown 语法后，中文每字记 1、英文按词记 1）、excerpt 非空、meta_desc 非空、",
		"标题 8~120 字符。不达标返回 422：",
		"",
		"```json",
		`{"error":"quality_gate","failures":["body_too_short (380/400)","excerpt_missing"]}`,
		"```",
		"",
		"收到 quality_gate 时按 failures 逐项补齐再重试；也可以先存草稿（草稿不校验）。",
		"",
		"## 单篇 SEO 覆盖（robots / canonical）",
		"",
		"update 请求体支持 `robots_override`（如 `\"noindex, follow\"`）与 `canonical_override`（必须是 http(s) 绝对 URL，",
		"否则 422 `invalid_canonical`）；传空串可清除覆盖。CLI 里用 `update ... --robots \"...\" --canonical <url>` 透传。",
		"典型用法：转载/同步内容把 canonical 指向原文；活动页临时 noindex。",
		"",
		...
		"",
		"- `node scripts/gcms.js search-stats --days 28 --limit 100 --group query`：Search Console 按搜索词聚合的点击、曝光、CTR 与平均排名（`GET /stats/search`）。",
		"- `--group` 支持 `query_page`（默认）、`query`、`page`、`date` 和 `total`；`total` 是精确区间汇总，`date` 是每日趋势，`--fresh` 可绕过一小时缓存。",
		"- `node scripts/gcms.js search-stats --days 28 --group page --compare`：另拉紧前等长区间并按当前维度合并，每行追加 `prev_clicks/prev_impressions/prev_ctr/prev_position`（前期无该项为 null）；`date` 不支持 compare。",
		"- `node scripts/gcms.js traffic-stats --days 7`：GA 活跃用户、会话、互动率与平均互动时长汇总（`GET /stats/traffic`）。",
		"- `node scripts/gcms.js page-stats --days 7 --limit 50`：GA 按页面路径返回活跃用户、会话、互动率与平均互动时长（`GET /stats/pages`），活跃用户降序。",
		"- `node scripts/gcms.js analytics-stats --days 30 --limit 200 --group sources`：GA 渠道与来源 / 媒介；`group` 也支持 `geography`（国家 / 地区）、`devices`（设备 / 系统 / 浏览器）和 `trend`（每日趋势），统一返回四项流量质量指标（`GET /stats/analytics`）。",
		"- `node scripts/gcms.js tg-stats`：Telegram 频道订阅数（`GET /stats/telegram`），返回 `{ok,members}`。",
		"",
		"compare 示例返回行：",
		"",
		"```json",
		`{"query":"gcms 教程","page":"https://example.com/zh/posts/guide/","clicks":12,"impressions":340,"ctr":0.0353,"position":9.4,"prev_clicks":5,"prev_impressions":210,"prev_ctr":0.0238,"prev_position":14.2}`,
		"```",
		"",
		"典型用法：用 `search-stats --group query` 找高曝光低点击或平均排名 4~20 的搜索词，",
