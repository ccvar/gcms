...
func automationStarterBriefMarkdown(opts automationSkillOptions) string {
	...
	return strings.Join([]string{
		...
		"- 如果有媒体上传权限和可用素材，先用 `POST /media` 上传图片，把返回的 `url` 写入 `cover_image` 或正文 Markdown 图片。",
...
func automationStarterWritePrompt() string {
	return strings.Join([]string{
		...
		"- 有图片素材和媒体权限时，先 `POST /media` 上传，文章必须写入 `cover_image`；需要展示操作步骤时，在正文合适位置插入 Markdown 图片。",
...
func automationStarterPromptExamples(opts automationSkillOptions) string {
	return strings.Join([]string{
		...
		"## 要求深度文章和配图",
		"",
		"第一批文章不要写成短说明。每篇文章先列搜索意图、目标读者、主关键词、建议字数、封面图来源和正文配图需求；有素材时先上传图片并写入 cover_image，教程类文章正文里要插入真实截图。",
		"",
		"## 缺图时先标注",
		"",
...
func automationStarterSkillMarkdown(apiBase string) string {
	return strings.Join([]string{
		...
		"- 有媒体权限和素材时，先 `POST /media` 上传图片，把返回 URL 写入 `cover_image` 或正文 Markdown 图片。",
