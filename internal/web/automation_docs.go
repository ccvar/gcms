	...
	paths := map[string]any{
		...
		"/stats/search": map[string]any{
			"get": map[string]any{
				...
				"description": "查询参数：days=28（钳制 1..90）、limit=100（钳制 1..1000）、compare=1（可选）。按 query+page 维度返回 {ok,days,property,rows:[{query,page,clicks,impressions,position}]}，结果缓存 1 小时。compare=1 时服务端另拉「紧前等长区间」数据并按 query+page 合并，每行追加 prev_clicks/prev_impressions/prev_position（前期无数据置 null），响应带 compare:true。未接入 Search Console 时返回 400 search_console_not_connected。典型用法：找平均排名 8~20 的搜索词，优化对应旧文；带 compare 观察优化后的涨跌。",
				"responses":   map[string]any{"200": map[string]any{"description": "OK"}},
			},
		},
		...
		"/stats/pages": map[string]any{
			"get": map[string]any{
				...
				"description": "查询参数：days=7（钳制 1..90）、limit=50（钳制 1..1000）。GA4 按 pagePath 维度返回 {ok,days,property,rows:[{path,active_users,sessions}]}（活跃用户降序），结果缓存 1 小时。未接入 Google Analytics 时返回 400 analytics_not_connected。典型用法：看哪些页面在涨/跌，配合 /stats/search 定位要优化的旧文。",
				"responses":   map[string]any{"200": map[string]any{"description": "OK"}},
			},
		},
	}
...
func automationSkillMarkdown(apiBase string) string {
	return strings.Join([]string{
		...
		"",
		"```json",
		`{"query":"gcms 教程","page":"https://example.com/zh/posts/guide/","clicks":12,"impressions":340,"position":9.4,"prev_clicks":5,"prev_impressions":210,"prev_position":14.2}`,
		"```",
		"",
