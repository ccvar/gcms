...
func automationStarterReadme(opts automationSkillOptions) string {
	...
	lines = append(lines,
		...
		"5. 规划确认后，再复制 `第二步-审核后写入草稿.md`，让 AI 分批写入：站点文案 -> 导航 -> 分类 -> 页面/文章/链接。",
...
func automationStarterRequirementsTemplate() string {
	return strings.Join([]string{
		...
		"",
		"- 第一轮是否只允许规划：是",
		"- 允许 AI 写入哪些内容：站点文案 / 导航 / 分类 / 页面 / 文章 / 链接 / 媒体",
		"- 默认内容状态：草稿",
		"- 是否允许发布：否",
	}, "\n") + "\n"
}
...
func automationStarterPlanPrompt() string {
	return strings.Join([]string{
		...
		"7. 第一批文章选题",
		"8. 第一批资源链接方向",
		"9. 会新增什么、会修改什么、哪些内容保持草稿",
		"10. 风险提醒，包括品牌、金融、医疗、法律、版权、隐私和夸大承诺",
		"",
...
func automationStarterSkillMarkdown(apiBase string) string {
	return strings.Join([]string{
		...
		"5. 用户明确确认后再分批写入：站点文案 -> 导航 -> 分类 -> 页面 -> 文章 -> 链接。",
		"6. 所有内容默认 `status: draft`。",
		"7. 完成后列出每条内容的 id、slug、语种、状态和需要人工复核的点。",
		"",
		"## 边界",
		"",
		"- 不要删除内容。",
